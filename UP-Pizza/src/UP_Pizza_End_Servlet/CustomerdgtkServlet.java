...
public class CustomerdgtkServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public CustomerdgtkServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int uid = cus.getId();
		//int uid = 1;
		String did = request.getParameter("idur");
		String info = request.getParameter("info");
		String fid = request.getParameter("fid");
		DingdanDAO dg = new DingdanDAO();
		OutinfoDAO fd = new OutinfoDAO();
		Outinfo fo = new Outinfo();
		ArrayList<Dingdan> Dg = dg.selectByDId(uid, did,fid);
		boolean f = false;
		for (Dingdan d : Dg) {
			f = fd.foodtkAdd(d.getId(), d.getQty(), d.getPrice(), info);
		}
		out.print(f);
		out.flush();
		out.close();
